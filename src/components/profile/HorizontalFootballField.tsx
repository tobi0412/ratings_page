...
export default function HorizontalFootballField({ selectedPositions, design }: Props) {
  console.log("HorizontalFootballField rendering design:", design);
  ...
  return (
    ...
    >
      ...
      <div className="absolute left-1/2 top-0 w-[1px] h-full" style={{ background: lineColor }}></div>
      {/* Center circle */}
      <div 
        className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full border border-solid" 
        style={{ width: '40px', height: '40px', borderColor: lineColor }}
      ></div>
      {/* Center dot */}
      <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-1 h-1 rounded-full" style={{ background: lineColor }}></div>

      {/* Left Penalty Area (Goalkeeper side) */}
      <div className="absolute left-0 top-1/4 w-1/6 h-1/2 border-r border-t border-b border-solid" style={{ borderColor: lineColor }}></div>
      {/* Left Goal Area */}
      <div className="absolute left-0 top-[35%] w-[6%] h-[30%] border-r border-t border-b border-solid" style={{ borderColor: lineColor }}></div>

      {/* Right Penalty Area (Attackers side) */}
      <div className="absolute right-0 top-1/4 w-1/6 h-1/2 border-l border-t border-b border-solid" style={{ borderColor: lineColor }}></div>
      {/* Right Goal Area */}
      <div className="absolute right-0 top-[35%] w-[6%] h-[30%] border-l border-t border-b border-solid" style={{ borderColor: lineColor }}></div>
